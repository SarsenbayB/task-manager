...
const SidebarItem = ({ item, pageName, setPageName }: any) => {
  ...
  const [clientPageName, setClientPageName] = useState(pageName);
    ...
    <>
      <li>
        ...
        >
          ...
          {item.children && (
            <svg
              ...
              className={`absolute right-3.5 top-1/2 -translate-y-1/2 fill-current ${clientPageName! !== item.label.toLowerCase() ? "rotate-180" : ""
                }`}
              width="22"
              height="22"
              viewBox="0 0 22 22"
        ...
        {item.children && (
          ...
          <div
            className={`translate transform overflow-hidden ${!hydrated || clientPageName !== item.label.toLowerCase() ? "hidden" : ""
              }`}
          >
